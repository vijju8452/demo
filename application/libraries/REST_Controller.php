...
abstract class REST_Controller extends CI_Controller
{
     ...
     * @var string|null
     */
    protected $rest_format          = 'json';   //Setting a default format : Durgesh
    ...
    */
	...
	// Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        ...
        //oauth validations
        //$this->load->helper('verify_access_helper');
        //$this->_oauth_validation_check();
    }
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // url: http://stackoverflow.com/questions/28526823/codeigniter-rest-server-fail-to-output-longer-json
...
//        }
    ...
    //To verify the access token passed in request header or not
    protected function _oauth_validation_check()
    {
        $this->load->library('resource');     
        $returnResponse=array();
        $response = $this->resource->verifyAccessToken();
        if(!$response){
            $returnResponse['response']['messages'][]  = ERROR_INVALID_ACCESS;
            $this->response($returnResponse, 401);
        }
